...
&emsp;&emsp;**当前方法也是在网上找的，找不到原作者了，如有侵权请告知**

1、表单验证 js（**WxValidate**）
...
2、需要验证页面引入以上（**WxValidate**）js

> `import WxValidate from '../lib/WxValidate.js'`

> 引用名：`WxValidate `

> 引用路径：`../lib/WxValidate.js`

在引入的 js 的 data 中定义一个对象，如：

```html
<form bindsubmit="formSubmit">
  <view class="section">
  <input name="CityID" placeholder="测试输入"></input>
</view>
  <view class="section">
  <input name="Name" placeholder="测试输入"></input>
</view>
  <view class="section">
  <input name="Year" placeholder="测试输入日期"></input>
</view>
  <view class="section">
  <input name="FrName" placeholder="测试输入"></input>
</view>
  <view class="section">
  <input name="FrTel" maxlength="11" type ='number'  placeholder="测试输入电话号码"></input>
</view>
   <button style='width:50%;background-color:#52b8f5;color:white;' formType="submit">保存</button>
</form>
```
...
```js
  ...
  formSubmit(e)
  {
    ...
    if (!this.WxValidate.checkForm(params)) {//获取表单信息进行验证
...
**目前大概就是酱紫**，我写个[代码段](https://github.com/Quietly-20201113/wx-Validate.git)吧 GitHub 的。
